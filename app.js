...
function getOrCreateStatus() {
  ...
  if (!el) {
    ...
    el.id = 'statusMsg';
    el.style.cssText = 'display:none;color:#c00;padding:12px 16px;background:#fff3f3;border:1px solid #fcc;border-radius:8px;margin:16px auto;max-width:800px;font-weight:500;';
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (!generateIdeasBtn) {
    showStatus('JS loaded but buttons not found. Reload the page.');
    return;
  }
  ...
  generateIdeasBtn.addEventListener('click', async function() {
    ...
    if (!niche) { showStatus('Please enter a niche first.'); return; }
  ...
  document.getElementById('copyIdeas').addEventListener('click', function() {
    ...
    navigator.clipboard.writeText(text).then(function() { showStatus('Copied to clipboard!'); });
  });
